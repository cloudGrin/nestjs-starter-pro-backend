...
describe('PermissionsGuard', () => {
  ...
  describe('canActivate', () => {
    ...
    describe('无权限要求的接口', () => {
      it('当接口未配置权限要求时，应该直接放行', async () => {
        ...
        expect(logger.debug).toHaveBeenCalledWith(
          expect.stringContaining('当前接口未配置权限要求')
        );
      });

      it('当权限要求为空数组时，应该直接放行', async () => {
        // Arrange
    ...
    describe('普通用户权限检查', () => {
      ...
      it('当用户拥有所需权限时，应该放行', async () => {
        ...
        // Mock withLock 直接执行回调
        cacheService.withLock.mockImplementation(async (key, fn) => {
          return await fn();
        });
  ...
  describe('getUserPermissions - QueryBuilder字段名验证', () => {
    it('应该使用正确的字段名（isActive 而非 is_active）', async () => {
      ...
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('p.isActive = :isActive', { isActive: true });
      expect(mockQueryBuilder.andWhere).toHaveBeenCalledWith('r.isActive = :isActive', { isActive: true });
  ...
  describe('clearUserPermissionsCache', () => {
    it('应该清除指定用户的权限缓存', async () => {
      ...
      expect(logger.debug).toHaveBeenCalledWith(
        expect.stringContaining('清除用户权限缓存完成')
      );
    });
  });
});
