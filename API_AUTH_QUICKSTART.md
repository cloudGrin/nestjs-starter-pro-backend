...
```bash
# 方式1：使用TypeORM CLI生成迁移
npm run typeorm migration:generate -- -n AddApiAuthTables

# 方式2：直接执行SQL文件
mysql -u root -p home < src/migrations/create-api-auth-tables.sql
```
...
```
# API调用总数
api_calls_total{app="my-app", endpoint="/users", method="GET"} 1234

# API响应时间
api_response_time_seconds{app="my-app", endpoint="/users", quantile="0.99"} 0.156

# 错误率
api_errors_total{app="my-app", status="500"} 12
```
